...
def courbe_roc_auc(labels_np, probabilities_np, n_bootstrap, alpha):
    """
    Affiche la courbe ROC, calcule l'AUC et son intervalle de confiance.
    """
    ...
    for _ in range(n_bootstrap):
        ...
        probs_boot = probabilities_np[indices]
        if len(np.unique(labels_boot)) < 2:
            continue
        fpr_b, tpr_b, _ = roc_curve(labels_boot, probs_boot, pos_label=1)
        aucs_boot.append(auc(fpr_b, tpr_b))
    ic_bas = np.percentile(aucs_boot, 100 * alpha / 2)
    ic_haut = np.percentile(aucs_boot, 100 * (1 - alpha / 2))
    ...
    print(f"AUC = {roc_auc:.4f} [IC 95% : {ic_bas:.4f} - {ic_haut:.4f}]")
